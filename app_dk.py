...
def roto_grinders():
    ...
    fdf['Player'] = fdf['Player'].replace('Matthew Beaty','Matt Beaty')
    return fdf
...
def projected_df(file_path):
    ...
    fdf['Player'] = fdf['Player'].str.replace('[^A-Za-z\s]', '', regex=True)
    fdf['Player'] = fdf['Player'].apply(lambda x: ' '.join(x.split(' ')[:2]))
    ...
    final_columns = [col for col in ['team','Name','Roster Position','cost','hr','rbi',
                                 ...
                                 'Injury Indicator','Batting Order'] if col in projDF.columns]
...
def roto_grinders_draftkings():
    ...
    ddf['Player'] = ddf['Player'].replace('Matthew Beaty','Matt Beaty')
    return ddf
...
def draftkings_projected_df(file_path):
    ...
    ddf['Player'] = ddf['Player'].str.replace('[^A-Za-z\s]', '', regex=True)
    ddf['Player'] = ddf['Player'].apply(lambda x: ' '.join(x.split(' ')[:2]))
    ...
    # Add the 'Player Triples over 0.5' column only if it exists in DKDF
    ...
    final_columns = [col for col in ['team','Name','Roster Position','cost','hr','rbi',
                                         'run','sb','single','double','triple','walk',
                                     'Injury Indicator','Batting Order'] if col in DKDF.columns]
...
def process_golf_data(file_path,tournament='WM Phoenix Open'):
...
def dk_process_golf_data(file_path,tournament='WM Phoenix Open'):
    pga_futures = get_pga_futures(tournament_name=tournament)
...
def process_file_ncaaf_fd():
    ...
    final_proj = ncaa_process_data_fd(file_path=filepath,tournament='ZOZO Championship')  # Assuming you're processing the DraftKings file the same way
